...
class Teep(models.Model):
    user = models.ForeignKey(
        User, related_name='teeps',
        on_delete=models.DO_NOTHING
    )
    body=models.CharField(max_length=200)
    created_at = models.DateTimeField(auto_now_add=True)
    likes = models.ManyToManyField(User, related_name="teep_like", blank=True)
    ...
    def __str__(self):
        return (
            f"{self.user} "
            f"({self.created_at:%Y-%m-%d}): " # for hour and minute just add '%H:%M'
            f"{self.body}..."
        )
...
def create_profile(sender, instance, created, **kwargs):
    if created:
        ...
        user_profile.save()
# This will follow user self  when a new account is created
